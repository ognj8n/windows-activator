...
""")
        ...
        if "will expire" in file:
            print(colored.green("[+]Izgleda kao da je Windows vec Aktiviran!"))
            time.sleep(3)
            sys.exit()
        else:
            print(colored.green("[+]Aktiviram Windows..."))
...
""")
            ...
            with open ("gotovo.txt","r",encoding="utf-8") as file:
                file=file.read()
            if "will expire" in file:
                print(colored.green("[+]Windows uspesno aktiviran!"))
                time.sleep(4)
                sys.exit()
            else:
                print(colored.red("[+]ERROR: Nisam uspeo aktivirati Windows na tvom PC!"))
                time.sleep(2)
                sys.exit()
